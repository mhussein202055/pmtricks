                        ...
                        <div class="portlet-body" style="margin-top: 35px;">
                            <div class="panel-group accordion scrollable" id="accordion2">
                                ...
                                @if(count($package_list))
                                    ...
                                    @if($pack->contant_type == 'question' || $pack->contant_type == 'combined')    
                                        <div class="panel panel-default">
                                            ...
                                            <div id="collapse_{{$pack->id}}" class="panel-collapse collapse">
                                                <div class="panel-body">
                                                    ...
                                                    <p>
                                                        <a href="{{route('attach.package', $pack->id)}}" class="btn blue">Go to Course </a> 
                                                    </p>
                                                    {{-- route('PremiumQuiz-st2', $pack->id) --}}
                                                    
                                                    @if(\Carbon\Carbon::parse(    \App\UserPackages::where('user_id', '=', Auth::user()->id)->where('package_id', '=',$pack->id)->get()->first()->created_at    )->addDays($pack->expire_in_days)->gte(\Carbon\Carbon::now()) )
                                                    <p>
                                                        Expire at : 
                                                        {{\Carbon\Carbon::parse(    \App\UserPackages::where('user_id', '=', Auth::user()->id)->where('package_id', '=',$pack->id)->get()->first()->created_at    )->addDays($pack->expire_in_days)->toFormattedDateString()}}
                                                    
                                                    </p>

                                                    @else
                                                    <p>
                                                        Expire at : {{ \Carbon\Carbon::parse( \App\PackageExtension::where('user_id', '=', Auth::user()->id)->where('package_id', '=', $pack->id)->orderBy('expire_at', 'desc')->get()->first()->expire_at )->toFormattedDateString()  }} 
                                                    </p>
                                                    @endif
                    ...
                    <div class="col-md-10 mx-auto form-1" >
                        ...
                        <div class="portlet-body" style="margin-top: 35px;">
                            <div class="panel-group accordion scrollable" id="accordion2">
                                    ...
                                    @if(count($video_package_list))
                                    @foreach($video_package_list as $pack)
                                    @if($pack->contant_type == 'video' || $pack->contant_type == 'combined')    
                                        <div class="panel panel-default">
                                            <div class="panel-heading">
                                                <h4 class="panel-title">
                                                    <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapse_video_{{$pack->id}}"> {{$pack->name}}</a>
                                                </h4>
                                            </div>
                                            <div id="collapse_video_{{$pack->id}}" class="panel-collapse collapse">
                                                <div class="panel-body">
                                                    <p> {!!$pack->description !!} </p>
                                                    
                                                    <div class="progress">
                                                        <div class="progress-bar" role="progressbar" style="width: @foreach($video_package_progress as $progress) @if($progress->package_id == $pack->id) {{$progress->progress}}%@endif @endforeach" aria-valuemin="0" aria-valuemax="100">@foreach($video_package_progress as $progress) @if($progress->package_id == $pack->id) {{$progress->progress}}%@endif @endforeach</div>
                                                    </div>
                                ...
                                @if(count($expire_package))
                                    @foreach($expire_package as $pack)
                                        ...
                                        <div class="panel panel-default">
                                            <div class="panel-heading">
                                                <h4 class="panel-title">
                                                    <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapse_expire_{{$pack->id}}"> {{$pack->name}}</a>
                                                </h4>
                                            </div>
                                            <div id="collapse_expire_{{$pack->id}}" class="panel-collapse collapse">
                                                <div class="panel-body">
                                                    <p> {!!$pack->description !!} </p>

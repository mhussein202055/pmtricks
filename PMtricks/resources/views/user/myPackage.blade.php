										...
										<div class="modal-content">
											...
											<div class="modal-header backgroud-style">
												...
												<div class="popup-logo">
													<img src="{{asset('indexassets/img/logo/p-logo.png')}}" alt="" style="width: 72 px;height: 68px">
												</div>
												<div class="popup-text text-center">
        ...
        <div class="short-filter-tab">
          <div class="shorting-filter  float-left"> <span><b>View</b> By</span>
            <select v-model="type" v-on:change="update_page">
              ...
              <option value="exam" @if($type == 'exam') selected @endif>Mock Exams</option>
              <option value="video" @if($type == 'video') selected @endif>Videos</option>
            </select>
          </div>
          ...
          <div class="shorting-filter float-right"> <span><b>Categories</b> By</span>
            <select v-model="cat" v-on:change="update_page">
              <option value="all" @if($cat == 'all') selected @endif >All Categories</option>
              @foreach(\App\Course::all() as $i)
              <option value="{{$i->id}}" @if($cat == $i->id) selected @endif >{{$i->title}}</option>
              @endforeach
            </select>
          </div>
        </div>
            ...
            <div id="tab1" class="tab-content-1 pt35">
              <div class="best-course-area best-course-v2">
                <div class="row">
                  ...
                  @foreach($exam_package_list as $i)
                  <div class="col-md-3">
                    <div class="course-item-pic-text " >
                      <div class="course-pic relative-position mb25"> 
                        <a href="{{route('attach.package', $i->id)}}">
                          <img src="{{ url('storage/package/imgs/'.basename($i->img))}}" alt=""> 
                        </a>
                      </div>
                      <div class="course-item-text">
                        <div class="course-title mt10 headline pb45 relative-position">
                          <h3><a href="{{route('attach.package', $i->id)}}">{{$i->name}} [Exam]</a></h3>
                        </div>
                        <div class="course-meta"> <span class="course-category bold-font"><a href="#">{{\App\Course::find($i->course_id)->title}}</a></span>
                          <div class="course-rate ul-li">
                            ...
                            @endphp
                            <ul>
                              <li><i class="fas fa-star" style="@if($total_rating >= 1) color: orange; @endif"></i></li>
                              <li><i class="fas fa-star" style="@if($total_rating >= 2) color: orange; @endif"></i></li>
                              <li><i class="fas fa-star" style="@if($total_rating >= 3) color: orange; @endif"></i></li>
                              <li><i class="fas fa-star" style="@if($total_rating >= 4) color: orange; @endif"></i></li>
                              <li><i class="fas fa-star" style="@if($total_rating >= 5) color: orange; @endif"></i></li>
                            </ul>
                          </div>
                        </div>
                        
                        <a href="{{route('attach.package', $i->id)}}">Get Started <i class="fas fa-caret-right"></i></a> 
                        
                      </div>
                    </div>
                  </div>
                  @endforeach
                  @foreach($video_package_list as $i)
                  <div class="col-md-3">
                    <div class="course-item-pic-text " >
                      <div class="course-pic relative-position mb25"> 
                        @if(\App\VideoProgress::where('user_id', Auth::user()->id)->where('package_id',$i->id)->where('complete', 1)->get()->first())
                            @php
                            ...
                            @endphp
                              <a href="{{ route('st4_vid', [$i->id, 'chapter', $last_video_chapter_id , $video_id_]) }}" class="btn blue">
                                  <img src="{{url('storage/package/imgs/'.basename($i->img))}}" alt="">
                              </a>

                        @else
                              <a href="{{ route('st4_vid', [$i->id, 'chapter', explode(',',\App\Packages::find($i->id)->chapter_included)[0] , \App\Video::where('chapter',explode(',',\App\Packages::find($i->id)->chapter_included)[0])->get()->first()->id]) }}" class="btn blue">
                                  <img src="{{url('storage/package/imgs/'.basename($i->img))}}" alt="">
                              </a>
                        @endif
                      ...
                      <div class="course-item-text">
                        <div class="course-title mt10 headline pb45 relative-position">
                            ...
                            @if(\App\VideoProgress::where('user_id', Auth::user()->id)->where('package_id',$i->id)->where('complete', 1)->get()->first())
                                    ...
                                    <a href="{{ route('st4_vid', [$i->id, 'chapter', $last_video_chapter_id , $video_id_]) }}" >
                                        {{ $i->name }} [Video]
                                    </a>
                                @else

                                    <a href="{{ route('st4_vid', [$i->id, 'chapter', explode(',',\App\Packages::find($i->id)->chapter_included)[0] , \App\Video::where('chapter',explode(',',\App\Packages::find($i->id)->chapter_included)[0])->get()->first()->id]) }}" >
                                        {{ $i->name }} [Video]
                                    </a>
                                @endif
                        ...
                        @if(\App\VideoProgress::where('user_id', Auth::user()->id)->where('package_id',$i->id)->where('complete', 1)->get()->first())
                              ...
                              @endphp
                            <a href="{{ route('st4_vid', [$i->id, 'chapter', $last_video_chapter_id, $video_id_]) }}" class="btn blue"> Continue learning <i class="fas fa-caret-right"></i></a>
                        @else
                            <a href="{{ route('st4_vid', [$i->id, 'chapter', explode(',',\App\Packages::find($i->id)->chapter_included)[0] , \App\Video::where('chapter',explode(',',\App\Packages::find($i->id)->chapter_included)[0])->get()->first()->id]) }}" class="btn blue">Start Course <i class="fas fa-caret-right"></i></a>
                        @endif
                  ...
                  <tr>
                    <td>
                      <div class="course-list-img-text">
                        <div class="course-list-img"> <img src="{{url('storage/package/imgs/'.basename($i->img_small))}}" alt=""> </div>
                        <div class="course-list-text">
                          <h3><a href="#">{{$i->name}} [Exam]</a></h3>
                          <div class="course-meta"> <span class="course-category bold-font"><a href="#">{{\App\Course::find($i->course_id)->title}}</a></span>
                            <div class="course-rate ul-li">
                              @php
                              $total_rating = 0;
                    ...
                    <td><a href="{{route('attach.package', $i->id)}}"><div class="course-type-list"> <span>Go To Courses</span> </div></a></td>
                    <td>{{\App\UserPackages::where('user_id',Auth::user()->id)->where('package_id', $i->id)->get()->first()->created_at}}</td>
                    <td>{{$i->expire_in_days}} days</td>
                  </tr>
                  @endforeach
        ...
        var app2 = new Vue({
            ...
            methods: {
              update_page: function(){
                window.location.replace('{{url('')}}/my_packages/'+this.type+'/cat/'+this.cat);
              }
            }

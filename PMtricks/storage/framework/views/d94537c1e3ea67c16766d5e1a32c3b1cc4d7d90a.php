        ...
        <div style="background: white; padding: 30px 15px; height: auto;">
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if(count(\App\PaymentApprove::all()) ): ?> 
                            <?php $__currentLoopData = \App\PaymentApprove::orderBy('created_at', 'desc')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $approve): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(\App\User::find($approve->user_id)): ?>
                                <tr>
                                    <td><?php echo e(\App\User::find($approve->user_id)->name); ?></td>
                                    <td><?php echo e(\App\Payments::find($approve->payment_id)->paypalEmail); ?></td>
                                    <td><?php echo e(\App\User::find($approve->user_id)->email); ?></td>
                                    <td><?php echo e(\App\Payments::find($approve->payment_id)->totalPaid); ?> $</td>
                                    <td><?php echo e(\App\Payments::find($approve->payment_id)->paymentMethod); ?></td>
                                    
                                    <td>
                                        <?php if(\App\Payments::find($approve->payment_id)->paymentMethod == 'check'): ?>
                                        <a target="_blank" href="<?php echo e(url('storage/payment_check/'.basename($approve->img))); ?>">Download</a>
                                        <?php else: ?>
                                        --
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo e($approve->created_at->diffForHumans()); ?></td>
                                    <td>
                                        <a href="<?php echo e(route('payment.approve.approve', $approve->id)); ?>">Approve</a>   |   
                                        <a style="color:brown;" href="<?php echo e(route('payment.approve.cancel', $approve->id)); ?>">Cancel</a>
                                    </td>
                                </tr>
                            <?php endif; ?>

...
class PremiumQuizController extends Controller
{
    ...
    public function st4_vid($package_id, $topic, $topic_id, $video_id){
        ...
        if($package->chapter_included != '' || $package->chapter_included != null){
            ...
            if( !empty($arr_chapters_id)){
                foreach($arr_chapters_id as $id){
                    array_push($chapters_inc_list, (int)$id);
        ...
        foreach($chapters_inc as $chapter){
            ...
            $total_time[0] += $chapter->total_hours;
            $total_time[1] += $chapter->total_min;
            $total_time[2] += $chapter->total_sec;
        ...
        if(!$security){
            return \redirect(route('index'))->with('error', 'Please, you must buy the package inorder to have access !');
        }
            
        // check videos in the same chapter
        $chapter_id = $video->chapter;
        $next_video = \App\Video::where('chapter', $chapter_id)->where('index_z','>', $video->index_z)->get()->first();
        ...
        if(!$next_video){
            ...
            if( ($key + 1) == count($chapters_inc_list) || $key == -1){
                ...
                $next_video = \App\Video::where('chapter', $chapter_id)->get()->first();
            }
